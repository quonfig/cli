...
- **fix(workspace bootstrap): the confirmation prompt names the workspace that will actually be pushed to.** It named the active profile's workspace whatever had been resolved, so with `QUONFIG_WORKSPACE` set it confirmed one workspace while the push landed in another. It now names whichever source chose the target (the env var, the pin, or the profile when the profile really is the resolved workspace), and the workspace id otherwise. (qfg-8p8i)
- **feat(qfg-verify): workspace `main` is append-only.** The pre-receive hook (`qfg-verify --git-hook`, the validator baked into the Quonfig Git server) now requires every update to `refs/heads/main` to fast-forward: the old oid must be non-zero and an ancestor of the new one, checked with `git merge-base --is-ancestor`. A force-push, a delete of `main` and a create of `main` by push are all rejected, with a message naming what to do instead — pull and rebase, or bring back earlier content as a **forward** commit (`git restore --source=<sha> --staged --worktree -- .`, commit, `qfg push`) — because a rewritten `main` silently wedges config delivery. Branches other than `main` stay rewritable, and content validation is unchanged. Enforcement is per repo and off until a workspace is named in the server's allowlist, so nothing changes for a workspace until it is rolled out. Operators can repair a corrupted repo with a declared, two-key bypass; the hook logs the reason. The hook also prints the `cli` sha it was built from as its first line. (qfg-jxml.21)
- **feat(workspace bootstrap): `qfg workspace bootstrap` keeps your history and never force-pushes.** Every workspace repo is provisioned with an initial commit, so the old bootstrap could only ever land by rewriting `main` (`--force-with-lease`, or `--force`) — and a rewritten `main` silently wedges config delivery: api-delivery updates its clone with `git pull --ff-only` and never recovers, and connected SDKs ignore a snapshot whose generation is not strictly greater than the one they hold. Bootstrap now fetches the workspace repo and replays your local history onto it **on a temporary worktree** (`git rebase --root --onto origin/main`), then pushes plainly. Your commits keep their authors and dates, your local branch is not moved, and nothing is rewritten. Because rebasing linearises merges, the replayed tip is required to have the same tree as your local tip (ignoring files the workspace seeded that you do not have); when it does not — a hand-resolved merge conflict is the case that matters — one final commit sets the tree to exactly your local content (`reconcile merge resolutions`, or `use the hosted workspace's quonfig.json` when that pin file is the only difference). If that cannot be done cleanly, nothing is pushed and your repository is untouched. If the replay hits a conflict it cannot resolve at all, it aborts, nothing is pushed, and the error points at `qfg push --dir <dir>` as the way to land the files without their history. The replay runs with your own `commit.gpgsign`, `core.hooksPath` and `rebase.autosquash` settings switched off — they are your existing commits being replayed onto a temporary worktree, not new work of yours to sign or to gate. (qfg-jxml.16)
- **fix(workspace bootstrap): "the workspace is empty" now means "it holds no documents", and a workspace that is in use is refused.** The old check was "the remote has no commits", which every real workspace fails: provisioning commits `README.md` and then `quonfig.json`. A workspace is treated as fresh when it has no documents under `configs/`, `feature-flags/`, `segments/`, `log-levels/`, `schemas/` or `schemas-protected/`; one that does is refused with a pointer to `qfg push --dir <dir>`, because landing a local history under a live workspace would overwrite edits made in the app — a schema authored in the UI just as much as a flag. If the check cannot read the workspace at all it fails, rather than reading as "empty". A workspace repo with no `main` branch is refused as not provisioned instead of having one pushed into existence. On a collision, your `README.md` wins and the workspace's `quonfig.json` wins — that file carries the pin naming the hosted workspace, so bootstrap no longer writes or pushes a pin commit of its own. (qfg-jxml.16)
- **feat(workspace bootstrap): the tree that is about to be pushed is validated, and the command tells you how to point your clone at the workspace.** Validation now runs a second time against the exact tree being pushed, which carries the workspace's `quonfig.json` rather than yours — so a config with an override for an environment that exists only in your local pin file fails with a readable message and nothing pushed, instead of being rejected by the server. (`--skip-validate` skips both passes.) Because bootstrap never moves your branch, the replayed commits have new ids and `qfg pull` cannot reconcile the two histories; on success the command now prints the recipe that can: `git branch pre-bootstrap && git fetch origin && git reset --hard origin/main` (the content is identical, and `pre-bootstrap` keeps your original history). It is also in `--json` as `localResetRecipe`. (qfg-jxml.16)
- **`--force` on `qfg workspace bootstrap` is now accepted and ignored.** The flag stays so pinned scripts do not fail on an unknown flag, and the command says it is ignoring it. There is no way to make bootstrap rewrite a workspace's `main`. (qfg-jxml.16)

## 0.0.74 - 2026-09-14

- fix(serve): `qfg serve` now actually honors `QUONFIG_ENVIRONMENT`, as its help text and docs have claimed since the command shipped. The `--environment` flag declared a `development` default, so oclif always populated it and the env-var fallback in the command body was unreachable: `QUONFIG_ENVIRONMENT=production qfg serve` served `development`. Resolution is now flag, then `QUONFIG_ENVIRONMENT`, then `development`. `qfg serve` hosts a service, so it reads the variable the same way an SDK does. (qfg-xk98)
- **fix(override): `qfg override` no longer reads `QUONFIG_ENVIRONMENT`.** `--env` is now required, and omitting it fails with `No environment specified. Pass --env=<env>. qfg override does not read QUONFIG_ENVIRONMENT.` Before, a variable left over from local development (a direnv `.envrc`, a CI image that sets it for the app under test) silently chose the environment an override was written to. The rule, now stated in the CLI docs: `QUONFIG_ENVIRONMENT` tells a _service_ which environment it is, and only `qfg serve` and `qfg run` read it; every command that reads or writes a workspace on your behalf takes the environment as an explicit flag. If a script relied on the old fallback, add `--env=<env>`. (qfg-xk98)

## 0.0.73 - 2026-09-02

- fix(set-default, set-rollout): both commands **destroyed every targeting rule** in the environment they wrote, while their help text and the docs claimed the opposite ("Any targeting rules or percentage rollouts you have configured are NOT affected"). They replaced the target environment's ENTIRE rule list with one unconditional rule, so a `qfg set-default my.flag --environment production --value false` in a CI script silently deleted the production targeting a team had built in the UI, reported success, and said nothing. Both commands are now **surgical**: they replace the value of the environment's fallback rule — the unconditional rule that decides what users receive when no targeting rule matches — **in place**, keep every other rule in order, and append a fallback only when the scope has none. The same applies to the `[Default]` scope. Every write now reports what it did: "Kept 2 targeting rule(s); matched users still receive their targeted value. To set for everyone, add --replace-targeting." (qfg-97z9, qfg-qjdm)
- feat(set-default, set-rollout): new `--replace-targeting` flag opts in to the old behavior explicitly — the environment collapses to a single unconditional rule and its targeting rules are deleted. The command says so ("Set production to `false` for everyone. Replaced 2 targeting rule(s); previous version 9f2c1ab.") and the interactive confirmation prompt names the count before you answer. The deleted rules stay in git history; `previous version` is the commit to restore from. This is the flag to use for a kill switch. (qfg-qjdm)
- feat(set-default, set-rollout): an environment with **no rules of its own** inherits the document's `default` rules, so writing it now starts from a copy of those rules ("production had no rules of its own; copied them from default.") instead of from an empty list. Without the copy, setting a fallback in an environment that had been inheriting silently stopped the inherited targeting from applying there — the exact shape of `our-config/feature-flags/app.workspaces.advanced-git-import.json`, whose only targeting lives in `default.rules`. This matches what the UI writes when an environment stops inheriting. (qfg-qjdm)
- feat(set-default, set-rollout): `--json` gained `keptTargetingRuleCount` on a surgical write, and `previousCommitSha` + `replacedTargetingRuleCount` under `--replace-targeting` (counts are omitted when zero). Every existing field is unchanged. (qfg-qjdm)
- fix(log-level): `qfg log-level --target ... --environment <env>` on an environment with no rules of its own now seeds that environment from a clone of `default.rules` before merging, the same as the two commands above. It previously started from an empty list, so adding a per-logger rule to an inheriting environment dropped every rule it had been inheriting. Merging itself is unchanged. (qfg-qjdm)
- fix(info, cleanup status): both readers now recognize **both** live catch-all spellings — `criteria: []` (what `qfg set-default` wrote before 0.0.72) and `criteria: [{"operator": "ALWAYS_TRUE"}]` — through one shared predicate. `qfg cleanup status` on a flag whose rules are `[targeting rule, bare catch-all]` reported the TARGETING rule's value as the fallback and counted both rules as overrides ("true (+2 override rules)" for a flag serving `false` to everyone but one address). (qfg-9kr8)
- fix(set-rollout): choosing `[Default]` as the environment now writes the document's `default` block, instead of creating an environment literally named `[Default]` that no SDK would ever match. (qfg-qjdm)
